...
public:
  ...
  Fruit() { pos = generateRandomPosition(); }
...
class Snake {
public:
  std::deque<Vector2> snake_body = {Vector2{10, 10}, Vector2{9, 10},
                                    Vector2{8, 10}};
  ...
  void grow() {
    Vector2 new_body = Vector2Subtract(snake_body[snake_body.size() - 1],
                                       snake_body[snake_body.size() - 2]);
    snake_body.push_back(
        Vector2Add(snake_body[snake_body.size() - 1], new_body));
  }

  void collidedWithWall(int &score, Fruit &fruit) {
    for (auto &pos : snake_body) {
      if (pos.x >= CELLS_WIDTH || pos.y >= CELLS_HEIGHT || pos.x < 0 ||
          pos.y < 0) {
        score = 0;
        direction = {1, 0};
        *this = Snake();
